...
static void CAN_GPIO_Config(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = CAN_TX_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;		         // 复用推挽输出12
  ...
  GPIO_InitStructure.GPIO_Pin = CAN_RX_PIN ;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;	             // 上拉输入11
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
//	TxMessage->ExtId = 0x200;
	...
	TxMessage.Data[0] = (int8_t)(iql >> 8);
TxMessage.Data[1] = (int8_t)(iql&0x00ff);
...
void update_angle(moter_angle* _angle,uint16_t angle_fb)
{
   _angle->encoder=angle_fb;
	...
	else 
	{
	  ...
	  _angle->encoder_init=1;
	
	}
	
   _angle->angle_offset=_angle->encoder_offset/8191.0f*360.0f;
   _angle->Last_encoder=_angle->encoder;
   _angle->total_encoder=_angle->round_cnt* 8191 +_angle->encoder - _angle->encoder_offset;
   _angle->angle=_angle->total_encoder/8191.0f*360.0f;
